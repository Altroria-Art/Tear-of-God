-- Extend in-app notifications for replies, template usage, followed creators,
-- trending rankings, and Community Average changes.
-- Existing rows are copied unchanged; the table rebuild is required because
-- SQLite cannot alter a CHECK constraint or add a foreign key in place.
PRAGMA foreign_keys = OFF;

CREATE TABLE notifications__v2 (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL,
  actor_id TEXT,
  type TEXT NOT NULL CHECK(type IN ('comment', 'follow', 'challenge', 'template_use', 'following_rank', 'trending', 'community_average')),
  ranking_id TEXT,
...
INSERT INTO notifications__v2 (
  id, user_id, actor_id, type, ranking_id, source_ranking_id,
  comment_id, template_id, is_read, created_at
)
SELECT id, user_id, actor_id, type, ranking_id, source_ranking_id,
       comment_id, NULL, is_read, created_at
FROM notifications;

DROP TABLE notifications;
ALTER TABLE notifications__v2 RENAME TO notifications;
