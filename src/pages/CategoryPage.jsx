...
const TIER_ROW_COLORS = { S: 'bg-red-400', A: 'bg-orange-400', B: 'bg-yellow-400' }
    ...
    <div className="flex items-center gap-2">
      ...
      <div className="flex flex-wrap items-center gap-2 overflow-hidden h-8">
        {items.slice(0, 3).map((item, idx) => (
          <span key={idx} className="rounded-md border border-gray-200 bg-white px-2.5 py-1 text-xs font-medium text-gray-700 whitespace-nowrap">
            {item}
          </span>
        ))}
      </div>
...
function TemplateCard({ template }) {
  ...
  return (
    <Link to={`/post/${template.id}`} className="block rounded-xl border border-gray-100 bg-white shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="px-4 pb-4">
        <h3 className="text-lg font-bold text-gray-900 line-clamp-1">{template.title}</h3>
        <div className="mt-2 flex items-center gap-2">
          <Avatar size="sm" name={template.profile?.username || 'Unknown'} src={template.profile?.avatar_url} />
          <span className="text-sm text-gray-500">@{template.profile?.username || 'User'}</span>
        </div>
      </div>
    </Link>
  )
  ...
  return (
    <main className="mx-auto min-h-screen max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <header className="mb-8 flex items-center gap-4">
        <Link to="/discover" className="rounded-full border border-gray-300 p-2 text-gray-600 transition-colors hover:bg-gray-100"><ArrowLeftIcon className="h-5 w-5" /></Link>
      ...
      {isLoading ? (
        ...
        <p className="text-center text-gray-500 py-10">ยังไม่มีโพสต์ในหมวดหมู่นี้</p>
      ) : (
