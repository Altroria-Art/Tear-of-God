...
export async function deleteOwnComment(request, db, user, isTemplate = false) {
  ...
  if (comment.user_id !== user.id) return Response.json({ success: false, error: 'Forbidden' }, { status: 403 });
  ...
  const statements = [
    ...
    db.prepare(`UPDATE ${table} SET parent_id = NULL WHERE parent_id = ?
      AND EXISTS (SELECT 1 FROM ${table} WHERE id = ? AND user_id = ?)`)
      .bind(id, id, user.id),
    db.prepare(`DELETE FROM ${table} WHERE id = ? AND user_id = ?`).bind(id, user.id),
  ];
